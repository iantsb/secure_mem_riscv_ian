...
// NOTE: Memory Controller Wrapper is placed between the memory bus and the tl2axi module.  All request are routed through the Memory Controller Wrapper.

//(* mark_debug = "true", keep = "true" *)

module MemoryControllerWrapper #(
    parameter config_pkg::rocket_cfg_t ROCKETCfg = build_config_pkg::build_config

) (
    input clock,
    input reset,  // On Posedge
    ...
    output [ROCKETCfg.TL_DATA_WIDTH-1:0] auto_ctl_in_d_bits_data,
    ...
    input [ROCKETCfg.TL_DATA_WIDTH-1:0] auto_in_a_bits_data,
    input auto_in_d_ready,
    output auto_in_d_valid,
    output [2:0] auto_in_d_bits_opcode,
    output [2:0] auto_in_d_bits_param, 
    output [2:0] auto_in_d_bits_size,
    output [7:0] auto_in_d_bits_source,
    output auto_in_d_bits_denied,
    output [ROCKETCfg.TL_DATA_WIDTH-1:0] auto_in_d_bits_data,
    ...
    output [ROCKETCfg.TL_DATA_WIDTH-1:0] auto_out_a_bits_data,
    output auto_out_d_ready,
    input auto_out_d_valid,
    input [2:0] auto_out_d_bits_opcode,
    input [2:0] auto_out_d_bits_param,
    input [2:0] auto_out_d_bits_size,
    input [9:0] auto_out_d_bits_source,
    input auto_out_d_bits_denied,
    input [ROCKETCfg.TL_DATA_WIDTH-1:0] auto_out_d_bits_data,
    input auto_out_d_bits_corrupt
    output [2:0] auto_ctl_in_d_bits_param,
    output       auto_ctl_in_d_bits_denied,
    output       auto_ctl_in_d_bits_corrupt,
);
  ...
  // --------------------------------
  // PORT ASSIGNMENTS - Control Port
  // --------------------------------

  assign auto_ctl_in_d_bits_opcode  = ctl_d_resp.opcode;
  assign auto_ctl_in_d_bits_param   = 3'b000; //src_d_data.param;
  assign auto_ctl_in_d_bits_size    = ctl_d_resp.size;
  ...
  // --------------------------------
  // PORT ASSIGNMENTS - Memory Ports
  // --------------------------------

  assign auto_out_a_bits_opcode = sink_a_data.opcode;
  assign auto_out_a_bits_param = 3'b000;
  ...
  assign auto_in_d_bits_param = 3'b000; //src_d_data.param;
  ...
  // -----------------------------------------
  // ASSIGMENTS - From wires to packed struct 
  // -----------------------------------------
  
  assign ctl_a_req.opcode  = auto_ctl_in_a_bits_opcode;
  assign ctl_a_req.param   = 3'b000;
  assign ctl_a_req.size    = auto_ctl_in_a_bits_size;
  ...
  assign sink_d_data.param   = 3'b000;
  ...
  secure_memory_controller #(
      ...
      .clk_i(clock),
       // Group Clock from system bus in posedge
      .rst_ni(~reset), 

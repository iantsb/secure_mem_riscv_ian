...
module secure_memory_controller #(
  ...
  // -----------------------------
  // Metadata controller for MEU counter/version requests
  // -----------------------------
  ...
  // ------------------------------------------------------------
  // Metadata controller and local metadata backing RAM.
  //
  // This services the MEU's META_READ_VERSION and META_ALLOC_VERSION
  // requests. Later this RAM can be replaced by a real metadata memory
  // path or merged with the BMT metadata path.
  // ------------------------------------------------------------
  metadata_controller_serial #(
    ...
    .req_valid_i     (meu_meta_req_valid),
    .req_ready_o     (meu_meta_req_ready),
    .req_op_i        (meu_meta_req_op),
    .req_addr_i      (meu_meta_req_addr),
    .req_lane_i      (meu_meta_req_lane),
    .req_wdata_i     ('0),
    .req_tag_i       ('0),

    .rsp_valid_o     (meu_meta_rsp_valid),
    .rsp_ready_i     (meu_meta_rsp_ready),
    .rsp_data_o      (md_rsp_data),
    .rsp_version_o   (meu_meta_rsp_version),
    .rsp_tag_o       (md_rsp_tag),
    .rsp_hit_o       (md_rsp_hit),
    .rsp_error_o     (meu_meta_rsp_error),
  ...
  // Routes TileLink requests and responses either through the MEU or bypass path.
  ...
  always_comb begin : route_resp_comb
    ...
    if (((sink_d_valid_i && sink_d_resp_i.source[9]) ? 1'b1 : 1'b0) ||
        (meu_src_d_valid && !sink_d_valid_i)) begin
      resp_pathway = MEU;
    end
    ...
    if (resp_pathway == BYPASS) begin
      ...
      src_d_resp_o.source = {2'b00, sink_d_resp_i.source[7:0]};
